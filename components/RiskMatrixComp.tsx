...
import { RiskControl, AuditEntity, TaskPlannerEntry } from '../types';
...
const RiskMatrix: React.FC<RiskMatrixProps> = ({ risks, entities, plannerData, areas, onAddArea, filterEntityName, onClearFilter, onUpdate }) => {
        ...
        <table className="w-full text-left border-collapse min-w-[2800px]">
          <thead>
            <tr className="bg-[#1a5f7a] text-white">
              <th className="p-4 text-xs font-bold uppercase tracking-wide border-r border-white/10 whitespace-nowrap min-w-[220px]">Entidad</th>
              <th className="p-4 text-xs font-bold uppercase tracking-wide border-r border-white/10 whitespace-nowrap min-w-[250px]">Alcance de Auditoría</th>
            ...
            {filteredRisks.map((risk) => (
              <tr key={risk.id} className="group hover:bg-slate-50 transition-colors">
                ...
                <td className="p-0 border-r border-slate-100">
                  <input
                    type="text"
                    className="w-full h-full p-4 text-sm text-slate-800 font-medium bg-transparent border-none focus:ring-2 focus:ring-blue-500/20 outline-none placeholder:text-slate-300"
