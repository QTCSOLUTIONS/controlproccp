...
const Schedule: React.FC<ScheduleProps> = ({ entities, onUpdatePhaseStatus, onUpdatePhase, onEditEntity }) => {
  ...
  // Usamos la fecha de inicio más temprana o la actual como en el componente original,
  // pero la referencia visual sugiere una fecha fija o calculada distinta.
  // Para alinear con aistudio (fixed 16 Feb 2026), podríamos usar eso,
  // pero para funcionalidad real es mejor mantener el cálculo dinámico u optar por la fija si el usuario lo prefiere.
  // Voy a mantener el cálculo dinámico de la versión anterior que es más robusto para un sistema real,
  // pero adoptando la estética de la referencia.

  const BASE_DATE = useMemo(() => {
    // Aistudio usa: new Date(2026, 1, 16);
    // Intentaremos usar la fecha de referencia de aistudio si encaja con los datos,
    // o volveremos a la calculada si no hay datos.
    // Dado que el usuario quiere "replicar", usaré la fecha fija de referencia para probar, 
    // o mejor, mantengo la lógica dinámica que ya funcionaba bien con los datos reales.
    // MANTENDRÉ LA LÓGICA DINÁMICA del componente original para no romper la visualización de datos existentes.
                            ...
                            return (
                              <button
                                ...
                                className={`absolute h-14 rounded-xl px-3 flex flex-col justify-center overflow-hidden shadow-sm border-l-4 group/bar transition-all hover:scale-[1.01] hover:brightness-95 active:scale-[0.98] pointer-events-auto z-10 ${phase.status === 'Completed' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' :
                                  phase.status === 'Execution' ? 'bg-blue-50 border-blue-500 text-blue-770' : // Corregido typo en referencia (posiblemente)
